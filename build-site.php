 ...
 * With OG/Twitter meta so thumbnails appear on FB, X, etc.
 */
...
function slugify($s){
  ...
  $s = trim($s, '-');
  $s = preg_replace('~-+~', '-', $s);
  return strtolower($s ?: 'post');
}
function pick($node,$tag){$n=$node->getElementsByTagName($tag);return $n->length?trim($n->item(0)->textContent):'';}
function firstAttr($node,$tag,$attr){
  $n=$node->getElementsByTagName($tag);
  if($n->length){$a=$n->item(0)->attributes;
...
function bigger_image($img){
  if(!$img)return $img;
  // Replace small GSMArena-style sizes like /-184x111/ with /-1920x1080/
  $img=preg_replace('~/-\d+x\d+/~','/-1920x1080/',$img);
...
foreach($items as $it){
  ...
  $img=bigger_image($img);

  $slug=slugify($title).'-'.substr(sha1($link),0,6);
